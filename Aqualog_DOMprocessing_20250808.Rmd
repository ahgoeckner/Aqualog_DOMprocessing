---
title: "Aqualog_DOMprocessing_20250808"
author: "A. Goeckner"
date: "`r Sys.Date()`"
...
#A. Directories, libraries, folder creation, data formatting
1. Setting wd and project names (testing)
```{r}
#Set wd as the main Aqualog  directory where EVERYTHING lives (script folders, raw datafolders)
setwd("/Users/audreygoeckner/OneDrive - University of Florida/Aqualog_processing")

#name of the project you are working in today
project_name <- "Howley_20240410"
...
#Pick a sample or samples that you want to designate as your "check". As we go through the code, instead of assessing figures for every sample (can take a lot of time for all plots to generate), this is a quick check to make sure that the code is working. Mkae sure to review all samples at some point to make sure none are unusual.
review_sample <- c("13", "9")
#review_sample <- "13"

#Today's date so that output files can have the date in it
output_date <- "20240410"

#optional - create output folder if you dont have one
dir.create(paste("./projects",project_name,"output",sep = "/"))
```

2. Libraries, working directory
```{r}
librarian::shelf(
  tidyr,
  plyr,
...
4. creat abs and eem folders if you dont already have them within the project folder
```{r}
...
#B. Importing processed EEMS, ABS, dilution step, removing questionable samples
1. Load EEM and ABS data
```{r}
...
#plot the entire eem_list sample
eem_overview_plot(eem_list)   


#use this to (plot the waterfall plots instead (not my fave but go for it)
eem_plot_3D(eem_list_forReview)

```

3. Create, fill in, and import DILUTION metatable
...
eem_metatemplate(eem_list, absorbance) %>%
  mutate(dilution = "1") %>% 
   write.csv(file = paste("./projects",project_name,dilution_sheet_name,sep = "/"), row.names = FALSE) #edit to fit your directory if needed

#Step 2: go open that dilution sheet and fill it in manually!!!!

#Step 3: read in edited dilution sheet
meta <-
  read.table(paste("./projects",project_name,dilution_sheet_name,sep = "/"),
    header = TRUE,
    sep = ",",
    dec = ".",
    row.names = 1)
...
4. Remove any samples with known issues such as poor data collection,
...
#C. Applying ABS corrections, EEM step size adjustments, blank subtraction

1. ABS data corrections
...
# View EEMs plots - DOES NOT WORK
#eem_overview_plot(test_eem_list, spp = 9, contour = TRUE)
...
#D. IFE correction, raman normalization, blank sample removal
...
3. Remove Blanks From Sample Set
...
#E. Raman and Rayleigh Scatter removal, scatter interpolation
...
eem_list_rem <-
  eem_remove_scattering_nonsym(
    ...
    type = "rayleigh",
    order = 1,
    width_above = 15, #change this and the line below if you want a thinner/thicker chunk removed
    width_under = 50
  )
...
#F. Applying dilution data and calculating fluorescent, absorbance indices
...
write.csv(slope_parms, row.names = FALSE, 
          file = paste("./projects/", project_name,"/output/","indices_abs_",output_date,".csv",sep = ""))
...
write.csv(eem_index, row.names = FALSE, 
          file = paste("./projects/", project_name,"/output/","indices_eem_",output_date,".csv",sep = ""))

```
#

#G. Saving R objects so you never have to do this ever again
```{r clean-up-and-save}
